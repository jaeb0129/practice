...
from data_loader import render_upload_ui, get_raw_df, get_master_df

# 사이드바 업로드 UI (한 번만 렌더링 → 모든 탭에 공유)
render_upload_ui()
...
def load_raw():
    ...
    data_pit = pd.merge(rslt, master.loc[:, ['PLER_TRKNG_ID', 'PLER_NAME', 'BKNO', 'TEAM_NM']],
                        left_on='PitcherId', right_on='PLER_TRKNG_ID', how='left')
    data_bat = pd.merge(rslt, master.loc[:, ['PLER_TRKNG_ID', 'PLER_NAME', 'BKNO', 'TEAM_NM']],
                        left_on='BatterId', right_on='PLER_TRKNG_ID', how='left')
    
    data_pit["name_bk"] = data_pit["PLER_NAME"] + "_" + data_pit["BKNO"].astype(str)
    data_bat["name_bk"] = data_bat["PLER_NAME"] + "_" + data_bat["BKNO"].astype(str)
...
pitchers, batters_dis, batter_hitrack = load_data()
master    = load_profile()
p_tools_df, b_tools_df = load_raw()

# ── 탭 ─────────────────────────────────────────────────────────────────────
tab_pitcher, tab_batter, tab_player_pitcher, tab_player_batter, tab_tools, tab_pitch, tab_draft = st.tabs([
...
# ══════════════════════════════════════════════════════
with tab_pitch:
    from tab_pitch import render as render_tab_pitch
    render_tab_pitch()
