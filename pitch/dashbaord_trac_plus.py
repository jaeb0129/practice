...
def release(Pitcher, Date):
    ...
    fig.update_layout(xaxis=dict(range=[150, -150], dtick=20, autorange=False, tickfont=dict(size=10)),
                      yaxis=dict(range=[80, 220], dtick=10, tickfont=dict(size=10)),
                      plot_bgcolor='whitesmoke',
                      width=500,
                      height=500,
                     title=f"{Pitcher} 릴리스 포인트", title_x=0.25)

    fig.add_trace(go.Scatter(x=[0, 0], y=[80, 220], mode='lines', line=dict(color='red'), showlegend=False))
    fig.add_trace(go.Scatter(x=[150, -150], y=[150, 150], mode='lines', line=dict(color='red'), showlegend=False))
...
result_pitch_trajectories = plot_pitch_trajectories(select_pitcher, select_date, time_points=[0.213, 0.100])
