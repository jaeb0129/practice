...
def _calc_flags(df: pd.DataFrame) -> pd.DataFrame:
    """TrackMan raw 데이터에 분석용 플래그 컬럼 추가."""
    df = df.copy()
...
def _build_grade(raw: pd.DataFrame) -> pd.DataFrame:
    """raw TrackMan → 선수별 Grade DataFrame 반환 (캐시)."""
    df = _calc_flags(raw)
    ...
    res["Grade_PLATE_DISCIPLINE"] = ((res["scale_WHIFF%"]  + res["scale_CHASE%"]) / 2).round(0).astype(int)
...
# 레이더에 표시할 툴 6개 (레이블 → 컬럼명)
RADAR_COLS = {
    "하드힛\n(HardHit)":  "scale_hardhit",
    "정타\n(ICR)":   "scale_icr",
    "배럴\n(Barrel)":     "scale_barrel",
    "컨택\n(헛스윙↓)":     "scale_WHIFF%",
    "존컨택":             "scale_ZONE_CONTACT%",
    "존밖\n반응":        "scale_CHASE%",
}
 
_COLORS = ["#38bdf8", "#818cf8", "#34d399", "#fb923c", "#f472b6", "#facc15"]
...
def _sec(title: str):
    st.markdown(
        f'<p style="font-size:.65rem;letter-spacing:.18em;text-transform:uppercase;'
...
def _render_radar(grade_df: pd.DataFrame, display_df: pd.DataFrame):
    _sec("선수 툴 스카우팅 레이더 (20-80 스케일) / 인플레이 10개 이상 대상 / 배럴%, 존컨택%는 최종 점수 계산에서 제외")
    ...
    for i, pid in enumerate(selected):
        ...
        tool_html = "".join([
            ...
            f'font-weight:700;color:{color}">{int(row[RADAR_COLS[c]])}</div>'
        ...
        <div style="background:#111827;border:1px solid #1e2d45;border-radius:10px;
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;
                      margin-bottom:.8rem">
            <span style="font-weight:700;font-size:1rem;color:#e2e8f0">{name}</span>
            <span style="font-size:.72rem;color:#6b7280">{team}</span>
          </div>
...
def _render_table(grade_df: pd.DataFrame, display_df: pd.DataFrame):
    ...
    pct_fmt = {c: "{:.1f}" for c in ["HardHit%","ICR%","BARREL%","WHIFF%","CHASE%"] if c in show_df.columns}
    pct_fmt["종합점수"] = "{:.1f}"
