...
import pandas as pd
import numpy as np
import db_func as dbf  # 사용자 정의 함수
...
def run_silent_mac_analysis(df, pitcher_name, target_hitters):
    df = df.copy()
    df['투수명_ID'] = df['투수명'] + '_' + df['PCER_ID'].astype(str)
    df['타자명_ID'] = df['타자명'] + '_' + df['BTER_ID'].astype(str)
    df = df.assign(EXIT_VELOCITY_CHECK=np.where(df['PICH_JUDG'] == '타격', df['HTNG_SPD'], np.nan))
    ...
    if 'PCER_LNR' in df.columns:
        ...
        if not pitcher_data.empty and not pitcher_data['PCER_LNR'].isna().all():
            pitcher_throws = pitcher_data['PCER_LNR'].mode().iloc[0]
    ...
    autopitchtype_to_group = {
        '직구': '직구', '싱커': '싱커', '커터': '커터', '슬라이더': '슬라이더',
        '스위퍼': '스위퍼', '체인지업': '체인지업', '스플리터': '스플리터', '커브': '커브'
    }
    ...
    for hitter in target_hitters:
        ...
        hitter_result["GB%"] = round(100 * total_gbs / total_bips_hittype, 1) if total_bips_hittype > 0 else np.nan
        hitter_result["FB%"] = round(100 * total_fbs / total_bips_hittype, 1) if total_bips_hittype > 0 else np.nan
        hitter_result["LD%"] = round(100 * total_lds / total_bips_hittype, 1) if total_bips_hittype > 0 else np.nan
...
def run_silent_mac_analysis_multiple_pitchers(df, pitcher_names, target_hitters):
    results_all = []
    group_breakdown_all = []
    df_all = []
    for pitcher_name in pitcher_names:
        result_df, group_breakdown_df, df_pitcher = run_silent_mac_analysis(df, pitcher_name, target_hitters)
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            st.error("CSV 파일을 읽는 데 실패했습니다. 파일 형식을 확인해 주세요.")
            df = None
    else:
        df = None
    ...
    with col1:
        st.subheader("투수 선택")
        selected_pitchers = st.multiselect("투수를 선택하세요:", available_pitchers)
    with col2:
        st.subheader("타자 선택")
        selected_hitters = st.multiselect("타자를 선택하세요:", available_batters)

    if st.button("맞대결 분석 시작", type="primary", use_container_width=True):
        if not selected_pitchers or not selected_hitters:
            st.warning("투수와 타자를 모두 선택해 주세요.")
        elif df is None:
            ...
            try:
                summary_df, breakdown_df, full_df = run_silent_mac_analysis_multiple_pitchers(
                    df, selected_pitchers, selected_hitters)
                if summary_df is not None and not summary_df.empty:
                    ...
                    st.warning("해당 조합에 대한 데이터가 충분하지 않습니다.")
            except Exception as e:
                st.error(f"분석 실패: {e}")
                import traceback
